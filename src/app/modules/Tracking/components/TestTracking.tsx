...
const TestTracking: React.FC = () => {
    ...
    const fetchMaterials = useCallback(async () => {
        setLoading(true);
        try {
            const res = await getAllMaterialTracking({ search, type: 'test' });
        ...
        <Content>
            <div className="mb-6">
                <h1 className="text-gray-900 fw-bold fs-2qx mb-1">
                    <i className="bi bi-clipboard2-check text-primary fs-2qx me-3"></i>
                    การติดตามการเทส (Test Tracking)
                </h1>
                <span className="text-muted fw-semibold fs-6">
                    ติดตามการใช้วัตถุดิบในการเทส — คลิกรายการเพื่อดูว่าไปอยู่ที่เอกสารไหนบ้าง ใช้ไปเท่าไร
                </span>
            </div>
                ...
                <>
                                ...
                                <div className="card-body d-flex align-items-center py-5">
                                    <div className="symbol symbol-50px me-4">
                                        <span className="symbol-label bg-light-info">
                                            <i className="bi bi-clipboard2-check text-info fs-2"></i>
                                        </span>
                                    </div>
                                    <div>
                                        <div className="fs-4 fw-bold text-gray-800">
                                            {materials.reduce((sum, s) => sum + s.used_in_testing, 0)}
                                        </div>
                                        <div className="text-muted fw-semibold fs-7">ใช้ในเทสทั้งหมด</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {materials.some(s => s.remaining_quantity <= 0) && (
                        <div className="alert alert-danger d-flex align-items-center py-3 mb-5">
                            ...
                            <div>
                                <span className="fw-bold">แจ้งเตือน:</span> มีวัตถุดิบที่หมดสต็อก กรุณาตรวจสอบก่อนดำเนินการเทส
                            </div>
                        </div>
                    )}
                                        ...
                                        {materials.map((item, idx) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="text-center text-primary fw-semibold">{item.used_in_production}</td>
                                                    <td className="text-center text-info fw-bold">{item.used_in_testing}</td>
