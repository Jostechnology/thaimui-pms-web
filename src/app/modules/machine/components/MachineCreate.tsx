...
const MachineCreate: React.FC = () => {
    ...
    const statusOptions = [
        ...
        { value: 'OFFLINE', label: 'ออฟไลน์ (Offline)', color: 'secondary', icon: 'bi-power' },
    ];
    ...
    const validate = (data: typeof formData): FormErrors => {
        ...
        if (!data.machine_code.trim()) {
            errs.machine_code = 'กรุณากรอกรหัสเครื่องจักร';
        } else if (data.machine_code.length > 50) {
            errs.machine_code = 'รหัสเครื่องจักรต้องไม่เกิน 50 ตัวอักษร';
        }
        if (!data.machine_name.trim()) {
            errs.machine_name = 'กรุณากรอกชื่อเครื่องจักร';
        }
        if (data.machine_description.length > 500) {
            errs.machine_description = 'รายละเอียดต้องไม่เกิน 500 ตัวอักษร';
        }
        return errs;
    };
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        // ถ้า field นี้ถูก touch แล้ว → validate ทันที (immediate feedback)
        if (touched[name]) {
        ...
        const confirm = await Swal.fire({
            ...
            html: `
                <div class="text-start fs-6">
                    <p class="mb-2"><strong>รหัส:</strong> ${formData.machine_code}</p>
                    <p class="mb-2"><strong>ชื่อ:</strong> ${formData.machine_name}</p>
                    <p class="mb-0"><strong>สถานะ:</strong> ${statusOptions.find(s => s.value === formData.status)?.label}</p>
                </div>
            `,
            icon: 'question',
        ...
        try {
            ...
            if (res.success) {
                await Swal.fire({
                    icon: 'success',
                    title: 'บันทึกสำเร็จ!',
                    text: `เครื่องจักร "${formData.machine_name}" ถูกเพิ่มเข้าสู่ระบบแล้ว`,
                    confirmButtonColor: '#3085d6',
                    timer: 2000,
    ...
    const selectClass = (fieldName: string) => {
        if (!touched[fieldName]) return 'form-select form-select-solid';
        return 'form-select form-select-solid is-valid';
    };
        ...
        <Content>
            ...
            <div className="d-flex align-items-center mb-6">
                <button
                    ...
                    title="กลับไปหน้ารายการเครื่องจักร"
                >
                    <i className="bi bi-arrow-left fs-4"></i>
                </button>
                <div>
                    <h3 className="fw-bolder mb-0">เพิ่มเครื่องจักรใหม่</h3>
                    <span className="text-muted fs-7">กรอกข้อมูลเครื่องจักรเพื่อลงทะเบียนในระบบ</span>
                </div>
            </div>
            ...
            <div className="row g-6">
                ...
                <div className="col-xl-8">
                    <form onSubmit={handleSubmit} id="machine-create-form">
                        ...
                        {/* ── Action Buttons (Sticky ด้านล่าง on mobile) ── */}
                        <div className="card">
